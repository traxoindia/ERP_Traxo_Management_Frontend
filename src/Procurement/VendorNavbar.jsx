...
function VendorNavbar() {
  ...
  const companyName = localStorage.getItem('company_name') || "Vendor Admin";
  const userEmail = localStorage.getItem('email') || "vendor@business.com";
  const vendor_id = localStorage.getItem('vendor_id') || "N/A";
    ...
    <nav className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          {/* Right Side: Dynamic Profile Info */}
          <div className="hidden md:ml-6 md:flex md:items-center space-x-4">
            <div className="relative" ref={dropdownRef}>
              ...
              {/* Desktop Dropdown */}
              {isProfileOpen && (
                <div className="origin-top-right absolute right-0 mt-3 w-56 rounded-xl shadow-xl py-2 bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100">
                  <div className="px-4 py-3">
                    <p className="text-sm text-gray-900 font-bold">Account Settings</p>
                    {/* DISPLAYING SAVED EMAIL */}
          ...
          <div className="flex items-center md:hidden">
            <button
              ...
              className="inline-flex items-center justify-center p-2 rounded-lg text-gray-500 hover:bg-gray-100"
            >
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white border-t border-gray-100">
          <div className="pt-2 pb-3 space-y-1">
            <Link to="/vendor-dashboard" className={mobileLinkStyle('/vendor-dashboard')} onClick={() => setIsMenuOpen(false)}>Dashboard</Link>
            <Link to="/my-products" className={mobileLinkStyle('/my-products')} onClick={() => setIsMenuOpen(false)}>My Products</Link>
          </div>
          <div className="pt-4 pb-3 border-t border-gray-200 bg-gray-50">
            <div className="flex items-center px-4">
              <div className="flex-shrink-0">
                <img className="h-10 w-10 rounded-full" src={`https://ui-avatars.com/api/?name=${companyName}&background=4f46e5&color=fff`} alt="" />
              </div>
              <div className="ml-3">
                <div className="text-base font-bold text-gray-800">{companyName}</div>
                <div className="text-sm font-medium text-gray-500">{vendor_id}</div>
