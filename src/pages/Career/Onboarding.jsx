...
const Onboarding = () => {
  ...
  // Work mode state for bulk finalization
  const [bulkWorkMode, setBulkWorkMode] = useState('OFFICE');
  ...
  // Updated bulk finalize with work mode parameter
  const handleBulkFinalize = async () => {
    const validRecords = readyList.filter(emp => !emp.employeeId?.includes("ERROR"));
    const ids = validRecords.map(emp => emp.employeeId || emp.employeeId);
    console.log("Finalizing IDs:", ids);
    console.log("Work Mode:", bulkWorkMode);
    ...
    setLoading(true);
    try {
      // Updated API call with workMode parameter
      const response = await axios.post(
        `${BASE_URL}/api/hr/bgv/bulk-finalize?workMode=${bulkWorkMode}`, 
        ids, 
        getHeaders()
      );
      ...
      alert(`Successfully finalized ${ids.length} employees with ${bulkWorkMode} mode.`);
      setActiveTab('history');
    } catch (err) {
      console.error("Finalize Error:", err);
      alert("Failed to finalize. Check console for details.");
  ...
  // Updated single finalize function with work mode
  const handleSingleFinalize = async (employee, workMode) => {
    const employeeId = employee.employeeId || employee.id;
    
    if (!employeeId || employeeId.includes("ERROR")) {
    ...
    try {
      // Updated API call with workMode parameter
      const response = await axios.post(
        `${BASE_URL}/api/hr/bgv/finalize/${employeeId}?workMode=${workMode}`,
        {},
        getHeaders()
      );

      console.log("%c [SUCCESS] Single Finalized:", "color: #10b981; font-weight: bold", response.data);

      // Remove from ready list and add to finalized list
      setReadyList(prev => prev.filter(emp => (emp.employeeId || emp.id) !== employeeId));
      setFinalizedList(prev => [...prev, employee]);

      alert(`Successfully finalized ${employee.fullName} with ${workMode} mode.`);
      
      await fetchDashboardData();
    } catch (err) {
      console.error("Single Finalize Error:", err);
      alert("Failed to finalize employee. " + (err.response?.data?.message || "Please try again."));
    } finally {
      setActionLoading(null);
    }
  };
  ...
  // Work Mode Selector Component
  const WorkModeSelector = ({ value, onChange, label, className = "" }) => (
    <div className={`${className}`}>
      {label && <label className="block text-sm font-medium text-slate-700 mb-2">{label}</label>}
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all"
      >
        <option value="OFFICE">🏢 Office (GPS Restricted - 150m radius)</option>
        <option value="WFH">🏠 WFH (No GPS check - work from anywhere)</option>
        <option value="REMOTE">💻 Remote (GPS recorded but not restricted)</option>
      </select>
    </div>
  );
        ...
        {activeTab === 'finalize' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center flex-wrap gap-4">
              ...
              {/* Bulk Work Mode Selector */}
              <div className="flex gap-3">
                <WorkModeSelector 
                  value={bulkWorkMode}
                  onChange={setBulkWorkMode}
                  label="Work Mode for Bulk Finalize"
                  className="w-64"
                />
                <button
                  onClick={handleBulkFinalize}
                ...
                <table className="w-full">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr>
                      <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500">Candidate</th>
                      <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500">Application ID</th>
                      <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500">Work Mode</th>
                      <th className="px-6 py-4 text-right text-xs font-semibold text-slate-500">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {readyList.map(e => {
                      const [singleWorkMode, setSingleWorkMode] = useState('OFFICE');
                      return (
                        <tr key={e.id || e.applicationId} className="hover:bg-slate-50">
                          <td className="px-6 py-4">
                            <div className="font-semibold text-slate-900">{e.fullName}</div>
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              {e.employeeId?.includes("ERROR") ? (
                                ...
                                <button
                                  onClick={() => handleSingleFinalize(e, singleWorkMode)}
                                  disabled={actionLoading === (e.employeeId || e.id)}
                                  className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                                >
                                  {actionLoading === (e.employeeId || e.id) ? (
                                    <Loader2 className="animate-spin w-4 h-4" />
                                  ) : (
                                    <>
                                      <CheckCircle size={14} />
            ...
            <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
              <h3 className="text-sm font-semibold text-blue-900 mb-2">Work Mode Policies:</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 bg-green-500 rounded-full"></span>
                  <span className="font-medium">Office:</span>
                  <span className="text-slate-600">GPS check with 150m radius restriction</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 bg-blue-500 rounded-full"></span>
                  <span className="font-medium">WFH:</span>
                  <span className="text-slate-600">No GPS check - work from anywhere</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 bg-purple-500 rounded-full"></span>
                  <span className="font-medium">Remote:</span>
                  <span className="text-slate-600">GPS recorded but no restriction</span>
                </div>
              </div>
            </div>
          </div>
